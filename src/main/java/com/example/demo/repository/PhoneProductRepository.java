...
public interface PhoneProductRepository extends JpaRepository<PhoneProduct, Long>{
	...
	@Query(value = "SELECT p.id_phone_product,p.name_phone_product,p.price,p.image FROM `phone_product` p WHERE p.id_phone_product NOT IN (SELECT a.id_phone_product FROM phone_product a WHERE id_phone_product = :idPhoneProduct) AND p.trang_thai = 1 ORDER BY RAND() LIMIT 4",nativeQuery = true)
